    ...
    <div className="p-6 max-w-lg mx-auto">
        ...
        <div className="grid gap-4">
            {groups.reverse().map((group, index) => (
                <div key={index} className="p-4 shadow-md border rounded-lg flex justify-between items-center min-h-[64px]">
                    <span className="font-medium">{group.grp_name}</span>
                    
                    {/* Buttons container for proper alignment */}
                    <div className="flex gap-3 items-center">
                        {isGroupAdmin(group) && (
                            <button onClick={() => openMemberPopup(group)} className="text-black hover:text-black">
                                <Plus size={20} />
                            </button>
                        )}
                        <button onClick={() => openGroup(group)} className="text-black py-1 h-11 rounded-lg transition">
                            View Group
                        </button>
                    </div>
                </div>
